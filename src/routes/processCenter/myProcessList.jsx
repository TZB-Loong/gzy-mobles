...
class MyProcessList extends Component {
  constructor(props) {
    super(props);
  ...
  querydata = () => {
    ...
    dispatch({
      ...
      if (!isfalse(myProcessModel.TodoTaskList)) {
        let TodoTaskList = myProcessModel.TodoTaskList;
                  ...
                  return (
                    <Item
                      ...
                      thumb={
                        <div
                          ...
                          style={{
                            backgroundColor:
                              item.processName == "支付审批" ? "#F29D39" : "#4B85F8"
                          }}
                        >
                      ...
                      onClick={()=>{
                        let goUrl = item.processName == "定标审批" ? (
                    ...
                    >
                      <Flex justify="between">
                        <div style={{ color: '#1890ff' }}>{item.processName}</div>
                        <span>
                            <div style={{ textAlign: "left" }}>
                              {item.isTurnDown == 1 ? (
                                <span className={styles.flowError}>被驳回</span>
                              ) : item.processState == 1 ? (
                                <span className={styles.flowProceed}>
                                  审批中
                                </span>
                              ) : item.processState == 2 ? (
                                <span className={styles.flowSucceed}>
                                  已完成
                                </span>
                              ) : (
                                ""
...
//     }
// };
export default connect(({ myProcessModel }) => ({ myProcessModel }))(
  MyProcessList,
  mapStateToProps
);
